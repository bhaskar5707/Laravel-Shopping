...
class ProductsController extends Controller
{
    public function addProduct(Request $request)
    {
        ...
        foreach ($categories as $cat) 
        {
            ...
            foreach ($sub_categories as $sub_cat) 
            {
                ...
                {
                     $categories_dropdown .="<option value='".$sub_cat2->id."'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;--->>&nbsp;&nbsp;".$sub_cat2->name."</option>";
                }
            }
        }
        //Display category end
        $brands = Brand::where(['status'=>1])->get();
        return view('admin.products.add-product')->with(compact('categories_dropdown','brands'));
    }
    function editProduct(Request $request , $id = null)
    {
        if($request->isMethod('post'))
        {
            ...
            Product::where(['id'=>$id])->update(['product_name'=>$data['product_name'],'product_code'=>$data['product_code'],'product_brand'=>$brand,'price'=>$data['price'],'description'=>$data['description'],'care'=>$data['care'],'image'=>$filename,'status'=>$status]);
    ...
    public function products($url=null)
    {
       ...
       if($categoryDetails->parent_id==0)
       {
           ...
           foreach ($subCategories as $key=>$subcat) {
            ...
            //Add For Subchild category
                $subCategories1 = Category::where(['parent_id'=>$subcat->id])->get();
               //$cat_ids = "";
               foreach ($subCategories1 as $key=>$subcat1) {
           ...
           //echo '<pre>';die;
           //print_r($cat_ids);die;
           //print_r($cat_ids1);die;
           //$productsAll= Product::whereIn('category_id',$cat_ids)->where('status',1)->get();
           $productsAll= Product::whereIn('category_id',$cat_ids)->orWhereIn('category_id',$cat_ids1)->where('status',1)->get();
       }
       else
       {
            $subCategories = Category::where(['parent_id'=>$categoryDetails->id])->get();
           ...
           //echo '<pre>';
           //print_r($cat_ids2);die;
           //if url is sub category url
           //$productsAll= Product::where(['category_id'=>$categoryDetails->id])->get();
            if(!empty($cat_ids2)){
            $productsAll= Product::where(['category_id'=>$categoryDetails->id])->orWhereIn('category_id',$cat_ids2)->get();}else{
                $productsAll= Product::where(['category_id'=>$categoryDetails->id])->get();
            }
       }
       ...
       $brands = Brand::where('status',1)->get();
       //dd($banners);
       return view('products.listing')->with(compact('categories','categoryDetails','productsAll','banners','brands'));
    }

    public function product_brands($url=null)
    {
       //Show 404 page if url does not exits
        $countBrand = Brand::where(['slug'=>$url,'status'=>1])->count();
        if($countBrand == 0)
        {
            abort(404);
        }
       ...
       $categories = Category::with('categories')->where(['parent_id'=>0])->get();
       $productsAll= Product::where(['product_brand'=>$url])->get();
       $banners = Banner::where('status',1)->get();
       $brands = Brand::where('status',1)->get();
       $brandDetails = Brand::where(['slug'=>$url])->first();
       $categoryDetails='';
       //dd($banners);
       return view('products.listing')->with(compact('categories','brandDetails','categoryDetails','productsAll','banners','brands'));
    }
    ...
    public function product($id= null)
    {
        ...
        return view('products.detail')->with(compact('productDetails','categories','productAltImages','total_stock','relatedProducts','brands'));
    }
    ...
    function groceryAndStaples(Request $request){
        
        $cate_id =  $_GET['cate_id']; //echo $cate_id;die;
        //Grocery & Staples Special
        $subCategories = Category::where(['parent_id'=>$cate_id])->get();
        //$cat_ids = "";
        foreach ($subCategories as $key=>$subcat) 
        {
            $cat_ids2[] =$subcat->id;
        }
        $gands_specials= Product::where(['category_id'=>$cate_id])->orWhereIn('category_id',$cat_ids2)->get();
        return view('grocery_staples')->with(compact('gands_specials'));
